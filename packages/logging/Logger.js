...
const LOG_SYMBOL = Symbol("mz logger raw log method");
const TIMERS_SYMBOL = Symbol("mz logger times");
...
class Logger {
	...
	timeLog(label) {
		...
		if (!prev) {
			throw new Error(`No such label '${label}' for Logger.timeLog()`);
		}
	...
	timeEnd(label) {
		const prev = this[TIMERS_SYMBOL] && this[TIMERS_SYMBOL].get(label);
		if (!prev) {
			throw new Error(`No such label '${label}' for Logger.timeEnd()`);
		}
		const time = process.hrtime(prev);
